<template>
  <a-modal width="800px" centered v-model="visible" :maskClosable="false" @cancel="hide" :title="!editable ? $t('add_form') : $t('update_form')">
...
export default {
  ...
  methods: {
    ...mapActions(['postWidgets', 'getAllWidgets', 'updateWidgets']),
    hide() {
      this.visible = false
      this.clear()
    },
    ...
    saveDate  () {
      this.$refs.brandCreate.validateForm().then(res => {
        console.log(res)
        this.loading = true
        this.postWidgets(res.data).then(res => {
